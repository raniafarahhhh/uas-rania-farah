...
function ListOfTask({ navigation, route }) {
  ...
  const fetchData = async () => {
    try {
      const response = await CApi.post('/action/find', {
        dataSource: 'Cluster0',
        database: 'puppet_uas',
        collection: 'task', // Change collection name to match your tasks collection
      });
        ...
        <TouchableOpacity key={index} style={styles.menuItem}>
          <View style={styles.body}>
            ...
            <View style={styles.textContainer}>
              <Text style={styles.bodyText}>Task Name: {task.taskName}</Text>
              <Text style={styles.bodyText1}>Description: {task.description}</Text>
              <View style={styles.textbody}>
                <Image
                  source={require('./assets/comment.png')}
                  style={styles.headerImage1}
                />
                <Text style={styles.bodyText2}>Status: {task.status}</Text>
              </View>
            </View>
          </View>
        </TouchableOpacity>
